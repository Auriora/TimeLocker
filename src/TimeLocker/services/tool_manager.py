...
class PerformanceProfile:
    ...
    """
    typical_throughput_mbps: Optional[float] = None
...
class ToolCapabilities:
    ...
    performance_characteristics: PerformanceProfile = field(default_factory=PerformanceProfile)
...
class ToolManager:
    ...
    def get_supported_tools(self) -> List[ToolInfo]:
        ...
        logger.info(f"Found {len([t for t in tools if t.is_available])} available tools")
        return tools
    ...
    def validate_job_compatibility(
        ...
        """
        logger.debug(f"Validating job {job.config.job_id} compatibility with {tool_type}")
    ...
    def _calculate_optimal_parallelism(
        ...
        max_parallel = capabilities.configuration_options.get('max_parallel_files', 8)
