    ...
    if not getattr(_TyperCliRunner, "_timelocker_mixstderr_patched", False):
        ...
        def _patched_invoke(
            ...
            **extra: object,
            ...
            if (
                exception
                and isinstance(exception, TypeError)
                and "mix_stderr" in str(exception)
            ):
                ...
                if not stdout_text:
                    ...
                    if stdout_bytes:
                        ...
                        setattr(
                            result,
                            "stdout_bytes",
                            captured_stdout.encode(charset, errors="replace"),
                        )
            except Exception:
                pass
            try:
                ...
                if not output_val:
                    ...
                    if output_bytes:
                        setattr(
                            result,
                            "output",
                            output_bytes.decode(charset, errors="replace"),
                        )
...
_ = sys.modules.setdefault(
    "TimeLocker.config.configuration_manager", _timelocker_config_manager_module
)
...
def _register_builtin_symbol(
    symbol_name: str, module_path: str, fallback: object | None = None
) -> None:
...
_register_builtin_symbol(
    "ConfigurationManager",
    "TimeLocker.config.configuration_manager",
    ConfigurationManager,
)
...
snapshots_app = typer.Typer(
    help="Snapshot operations", context_settings=CLI_CONTEXT_SETTINGS
)
snapshots_app.info.options_metavar = "<OPTIONS>"
repos_app = typer.Typer(
    help="Repository operations", context_settings=CLI_CONTEXT_SETTINGS
)
repos_app.info.options_metavar = "<OPTIONS>"
config_app = typer.Typer(
    help="Configuration management commands", context_settings=CLI_CONTEXT_SETTINGS
)
config_app.info.options_metavar = "<OPTIONS>"
credentials_app = typer.Typer(
    help="Credential management commands", context_settings=CLI_CONTEXT_SETTINGS
)
credentials_app.info.options_metavar = "<OPTIONS>"

# Create security sub-app
security_app = typer.Typer(
    help="Security management commands", context_settings=CLI_CONTEXT_SETTINGS
)
...
config_import_app = typer.Typer(
    help="Import configuration commands", context_settings=CLI_CONTEXT_SETTINGS
)
config_import_app.info.options_metavar = "<OPTIONS>"

config_export_app = typer.Typer(
    help="Export configuration commands", context_settings=CLI_CONTEXT_SETTINGS
)
config_export_app.info.options_metavar = "<OPTIONS>"

# Create migrate app for configuration migration and validation
migrate_app = typer.Typer(
    help="Configuration migration and validation commands",
    context_settings=CLI_CONTEXT_SETTINGS,
)
...
repos_credentials_app = typer.Typer(
    help="Repository credential management", context_settings=CLI_CONTEXT_SETTINGS
)
...
def cli_version(
    short: Annotated[
        bool, typer.Option("--short", help="Only print the version number")
    ] = False,
) -> None:
...
def cli_help(
    topic: Annotated[
        str | None,
        typer.Argument(
            help="Help topic (repos, backup, restore, policy, schedule, selections)"
        ),
    ] = None,
) -> None:
    """
    ...
    if topic is None:
        ...
        console.print(
            "TimeLocker provides comprehensive backup and restore capabilities with"
        )
        console.print("policy-based management, scheduling, and data selection.\n")

        console.print("[bold]Main Command Groups:[/bold]")
        console.print(
            "  [cyan]repos[/cyan]       - Repository management (create, list, validate)"
        )
        console.print(
            "  [cyan]backup[/cyan]      - Backup operations (run, status, list)"
        )
        console.print(
            "  [cyan]restore[/cyan]     - Restore operations (browse, files, full)"
        )
        console.print(
            "  [cyan]snapshots[/cyan]   - Snapshot management (list, show, delete)"
        )
        console.print(
            "  [cyan]policy[/cyan]      - Policy management (backup and retention policies)"
        )
        console.print(
            "  [cyan]schedule[/cyan]    - Scheduling automation (create, manage schedules)"
        )
        console.print(
            "  [cyan]selections[/cyan]  - Data selection templates (include/exclude patterns)"
        )
        console.print(
            "  [cyan]config[/cyan]      - Configuration inspection, import, export"
        )
        ...
        console.print(
            "  [cyan]monitor[/cyan]     - System monitoring and health checks"
        )
        ...
        console.print(
            "     timelocker selections create documents --include '~/Documents/**'\n"
        )
    ...
    if topic == "repos" or topic == "repository":
        ...
        console.print(
            "Repositories store your backup data. TimeLocker supports multiple"
        )
        console.print("repository backends including local, S3, B2, SFTP, and more.\n")

        console.print("[bold]Common Commands:[/bold]")
        console.print(
            "  [cyan]repos add[/cyan] <name> <uri>  - Add a new repository configuration"
        )
        console.print(
            "  [cyan]repos init[/cyan] <name>       - Initialize a repository at its location"
        )
        console.print(
            "  [cyan]repos list[/cyan]                 - List all repositories"
        )
        console.print(
            "  [cyan]repos show[/cyan] <name>          - Show repository details"
        )
        console.print(
            "  [cyan]repos validate[/cyan] <name>      - Validate repository connectivity"
        )
        console.print(
            "  [cyan]repos check[/cyan] <name>         - Check repository integrity"
        )
        console.print(
            "  [cyan]repos stats[/cyan] <name>         - Show repository statistics"
        )
        console.print(
            "  [cyan]repos remove[/cyan] <name>        - Remove a repository\n"
        )
        ...
        console.print(
            "  timelocker repos add s3-backup s3:s3.amazonaws.com/my-bucket/backup\n"
        )
        ...
        console.print(
            "  repos credentials remove <name>  - Remove stored credentials\n"
        )

    elif topic == "backup":
        console.print("\n[bold cyan]Backup Operations Help[/bold cyan]\n")
        console.print(
            "Backup operations create snapshots of your data in repositories.\n"
        )

        console.print("[bold]Common Commands:[/bold]")
        console.print(
            "  [cyan]backup create[/cyan]              - Create a backup using a selection template"
        )
        console.print(
            "  [cyan]backup status[/cyan]              - Show current backup status"
        )
        console.print("  [cyan]backup list[/cyan]                - List backup history")
        console.print(
            "  [cyan]backup cancel[/cyan] <job-id>     - Cancel a running backup\n"
        )

        console.print("[bold]Examples:[/bold]")
        console.print("  # Create a backup with a selection template")
        console.print(
            "  timelocker backup create --selection documents --repository myrepo\n"
        )
        console.print("  # Create a backup from direct paths")
        console.print(
            "  timelocker backup create /path/to/backup --repository myrepo\n"
        )
        ...
        console.print(
            "Snapshot commands inspect, compare, search, and clean up stored backups.\n"
        )

        console.print("[bold]Common Commands:[/bold]")
        console.print(
            "  [cyan]snapshots list[/cyan] --repository <name>    - List snapshots"
        )
        console.print(
            "  [cyan]snapshots show[/cyan] <id> --repository <name> - Show snapshot metadata"
        )
        console.print(
            "  [cyan]snapshots find[/cyan] <query>                 - Search for files across snapshots"
        )
        console.print(
            "  [cyan]snapshots diff[/cyan] <snap1> <snap2>         - Compare file changes"
        )
        console.print(
            "  [cyan]snapshots forget[/cyan] --keep-daily 7        - Apply retention policies"
        )
        console.print(
            "  [cyan]snapshots prune[/cyan]                        - Remove unreferenced data\n"
        )
        ...
        console.print("[bold]Common Commands:[/bold]")
        console.print(
            "  [cyan]restore browse[/cyan] <repo> <snapshot>        - Browse snapshot contents"
        )
        console.print(
            "  [cyan]restore files[/cyan] <repo> <snapshot> <paths> - Restore specific files"
        )
        console.print(
            "  [cyan]restore full[/cyan] <repo> <snapshot> <target> - Restore entire snapshot"
        )
        console.print(
            "  [cyan]restore list[/cyan] <repo>                      - List available snapshots"
        )
        console.print(
            "  [cyan]restore find[/cyan] <repo> <query>              - Search for files"
        )
        console.print(
            "  [cyan]restore diff[/cyan] <repo> <snap1> <snap2>      - Compare snapshots\n"
        )
        ...
        console.print(
            "  timelocker restore files myrepo latest /restore/path --include '*.txt'\n"
        )
        ...
        console.print(
            "Policies define backup and retention rules for automated operations.\n"
        )

        console.print("[bold]Backup Policies:[/bold]")
        console.print(
            "  [cyan]policy backup create[/cyan] <name>   - Create a backup policy"
        )
        console.print(
            "  [cyan]policy backup list[/cyan]            - List backup policies"
        )
        console.print(
            "  [cyan]policy backup show[/cyan] <id>       - Show policy details"
        )
        console.print(
            "  [cyan]policy backup delete[/cyan] <id>     - Delete a policy\n"
        )

        console.print("[bold]Retention Policies:[/bold]")
        console.print(
            "  [cyan]policy retention create[/cyan] <name> - Create a retention policy"
        )
        console.print(
            "  [cyan]policy retention list[/cyan]          - List retention policies"
        )
        console.print(
            "  [cyan]policy retention show[/cyan] <id>     - Show policy details\n"
        )
        ...
        console.print("[bold]Common Commands:[/bold]")
        console.print(
            "  [cyan]schedule create[/cyan] <name> <policy>  - Create a schedule"
        )
        console.print(
            "  [cyan]schedule list[/cyan]                     - List all schedules"
        )
        console.print(
            "  [cyan]schedule show[/cyan] <name>              - Show schedule details"
        )
        console.print(
            "  [cyan]schedule enable[/cyan] <name>            - Enable a schedule"
        )
        console.print(
            "  [cyan]schedule disable[/cyan] <name>           - Disable a schedule"
        )
        console.print(
            "  [cyan]schedule generate-scripts[/cyan] <name>  - Generate automation scripts\n"
        )
        ...
        console.print(
            "  timelocker schedule generate-scripts daily-2am --platform cron\n"
        )
        ...
        console.print(
            "Selection templates define which files to include or exclude in backups.\n"
        )

        console.print("[bold]Common Commands:[/bold]")
        console.print(
            "  [cyan]selections create[/cyan] <name>     - Create a selection template"
        )
        console.print(
            "  [cyan]selections list[/cyan]              - List all templates"
        )
        console.print(
            "  [cyan]selections show[/cyan] <name>       - Show template details"
        )
        ...
        console.print(
            "  [cyan]selections import[/cyan] <file>     - Import a template\n"
        )
        ...
        console.print(
            "Use config commands to inspect, diff, import, and export TimeLocker settings.\n"
        )

        console.print("[bold]Common Commands:[/bold]")
        console.print(
            "  [cyan]config show[/cyan]                           - Display the active configuration"
        )
        console.print(
            "  [cyan]config diff[/cyan] <file>                    - Compare a file against active settings"
        )
        console.print(
            "  [cyan]config import restic[/cyan]                  - Import environment variables as configuration"
        )
        console.print(
            "  [cyan]config import timeshift[/cyan]               - Convert Timeshift profiles into selections"
        )
        console.print(
            "  [cyan]config export[/cyan] <file>                  - Export configuration to JSON"
        )
        console.print(
            "  [cyan]config validate[/cyan]                       - Validate schema and dependencies\n"
        )
        ...
        console.print(
            "Credential commands securely store repository passwords and access tokens.\n"
        )

        console.print("[bold]Common Commands:[/bold]")
        console.print(
            "  [cyan]credentials set[/cyan] <repository>          - Store credentials for a repository"
        )
        console.print(
            "  [cyan]credentials list[/cyan]                      - Show stored credentials (names only)"
        )
        console.print(
            "  [cyan]credentials remove[/cyan] <repository>       - Delete stored credentials"
        )
        console.print(
            "  [cyan]credentials unlock[/cyan]                    - Unlock credential vault for automation\n"
        )
        ...
        console.print(
            "Security commands audit access, review compliance, and inspect protection settings.\n"
        )

        console.print("[bold]Common Commands:[/bold]")
        console.print(
            "  [cyan]security status[/cyan]                      - Show encryption, access, and session status"
        )
        console.print(
            "  [cyan]security audit[/cyan] --days 30             - View audit trail for recent events"
        )
        console.print(
            "  [cyan]security notifications[/cyan]               - Configure notification channels"
        )
        console.print(
            "  [cyan]security sessions[/cyan]                    - List or revoke active access sessions\n"
        )
        ...
        console.print(
            "Monitoring commands provide an operational dashboard for repositories and schedules.\n"
        )

        console.print("[bold]Common Commands:[/bold]")
        console.print(
            "  [cyan]monitor status[/cyan]               - Show system health summary"
        )
        console.print(
            "  [cyan]monitor operations[/cyan]           - List current or recent operations"
        )
        console.print(
            "  [cyan]monitor health[/cyan]               - Run repository health checks"
        )
        console.print(
            "  [cyan]monitor history[/cyan]              - Review historical backup activity"
        )
        console.print(
            "  [cyan]monitor stats[/cyan]                - Display aggregated statistics\n"
        )
        ...
        console.print(
            "Log commands inspect, filter, and clear TimeLocker CLI log files.\n"
        )

        console.print("[bold]Common Commands:[/bold]")
        console.print(
            "  [cyan]logs list[/cyan]                         - List available log files"
        )
        console.print(
            "  [cyan]logs tail[/cyan] --lines 200             - Show recent log lines"
        )
        console.print(
            "  [cyan]logs export[/cyan] <dest>                - Export logs for support"
        )
        console.print(
            "  [cyan]logs clear[/cyan]                        - Truncate cached logs\n"
        )
        ...
        console.print(
            "Reporting commands generate health, usage, and compliance summaries.\n"
        )

        console.print("[bold]Common Commands:[/bold]")
        console.print(
            "  [cyan]reports generate[/cyan] backup-history --days 14  - Backup history report"
        )
        console.print(
            "  [cyan]reports generate[/cyan] storage-usage             - Storage utilization report"
        )
        console.print(
            "  [cyan]reports generate[/cyan] performance --format json - Performance report\n"
        )

        console.print("[bold]Examples:[/bold]")
        console.print(
            "  timelocker reports generate backup-history --days 30 --output report.md"
        )
        ...
        console.print(
            "Migrate commands validate exported configurations before import.\n"
        )

        console.print("[bold]Common Commands:[/bold]")
        console.print(
            "  [cyan]migrate validate[/cyan] <file> --show-changes     - Preview applied changes"
        )
        console.print(
            "  [cyan]migrate validate[/cyan] <file> --check-compatibility - Check version compatibility\n"
        )
        ...
        show_error_panel("Unknown Topic", unknown_topic_message)
        raise typer.Exit(1)
...
def cli_completion(
    shell: Annotated[
        str | None, typer.Argument(help="Target shell (bash, zsh, fish, powershell)")
    ] = None,
    install: Annotated[
        bool,
        typer.Option("--install", help="Install completion for the specified shell"),
    ] = False,
    uninstall: Annotated[
        bool,
        typer.Option(
            "--uninstall", help="Uninstall completion for the specified shell"
        ),
    ] = False,
    verify: Annotated[
        bool, typer.Option("--verify", help="Verify completion installation")
    ] = False,
) -> None:
    """
    ...
    if action_requested and shell is None:
        ...
        show_error_panel("Missing Shell Argument", missing_shell_message)
        raise typer.Exit(1)
    ...
    if shell is None:
        ...
        console.print(
            "  timelocker completion --install bash     # Install for specific shell\n"
        )
        ...
        console.print(
            "  timelocker completion --uninstall bash   # Remove completion\n"
        )
    ...
    if verify:
        ...
        if shell == "powershell":
            console.print(
                "[yellow]PowerShell completion verification not yet implemented[/yellow]"
            )
            ...
            if completion_file and completion_file.exists():
                console.print(
                    f"[green]✓[/green] Completion file exists: {completion_file}"
                )
                is_installed = True
            else:
                console.print(
                    f"[red]✗[/red] Completion file not found: {completion_file}"
                )
            ...
            if config["rc_file"] and config["source_line"]:
                ...
                if rc_file.exists():
                    ...
                    if config["source_line"] in rc_content:
                        console.print(
                            f"[green]✓[/green] Shell configuration updated: {rc_file}"
                        )
                    else:
                        console.print(
                            f"[yellow]⚠[/yellow] Shell configuration not updated: {rc_file}"
                        )
                        issues.append(f"Add '{config['source_line']}' to {rc_file}")
                        is_installed = False
                else:
                    console.print(
                        f"[yellow]⚠[/yellow] Shell configuration file not found: {rc_file}"
                    )
        ...
        if is_installed and not issues:
            ...
            console.print(
                "[bold yellow]Completion installation incomplete[/bold yellow]\n"
            )
            if issues:
                ...
                console.print()
            console.print(
                f"To install, run: [cyan]timelocker completion --install {shell}[/cyan]\n"
            )
    ...
    if uninstall:
        ...
        if completion_file and completion_file.exists():
            try:
                ...
                console.print(
                    f"[green]✓[/green] Removed completion file: {completion_file}"
                )
            ...
            if rc_file.exists():
                try:
                    ...
                    if len(new_lines) < len(lines):
                        ...
                        console.print(
                            f"[green]✓[/green] Removed source line from: {rc_file}"
                        )
                        removed_items.append("shell configuration")
                except Exception as e:
                    console.print(
                        f"[red]✗[/red] Failed to update shell configuration: {e}"
                    )
        ...
        if removed_items:
            ...
            show_success_panel("Completion Uninstalled", uninstall_message)
        else:
    ...
    if install:
        ...
        try:
            ...
            if shell == "powershell":
                console.print(
                    "[yellow]PowerShell automatic installation not yet implemented[/yellow]"
                )
            ...
            try:
                ...
                console.print(
                    f"  [red]✗[/red] Failed to generate completion script: {e}"
                )
                console.print(
                    f"  Run manually: [cyan]{config['generate_cmd']}[/cyan]\n"
                )
                raise
            ...
            if config["rc_file"] and config["source_line"]:
                ...
                if rc_file.exists():
                    ...
                    if (
                        config["source_line"] in rc_content
                        or "# TimeLocker completion" in rc_content
                    ):
                        console.print(
                            f"  [green]✓[/green] Already configured in {rc_file}\n"
                        )
                    else:
                        # Add source line
            ...
            show_success_panel("Installation Instructions", installation_message)
...
def config_import_restic(
    config_dir: Annotated[
        Path | None, typer.Option("--config-dir", help="Configuration directory")
    ] = None,
    config_file: Annotated[
        Path | None,
        typer.Option("--config-file", help="Optional configuration file to update"),
    ] = None,
    dry_run: Annotated[
        bool,
        typer.Option(
            "--dry-run", help="Preview changes without modifying configuration"
        ),
    ] = False,
    verbose: Annotated[
    ...
    try:
        ...
        if success_flag:
            ...
            show_error_panel(
                "Restic Import Failed",
                "Failed to import restic configuration.",
                result.errors,
            )
            raise typer.Exit(1)
        ...
        show_error_panel(
            "Restic Import Error", f"Failed to import restic configuration: {exc}"
        )
        if verbose:
...
def config_import_timeshift(
    ...
    config_file: Annotated[
        Path | None,
        typer.Option("--config-file", help="Path to Timeshift configuration file"),
    ] = None,
    dry_run: Annotated[
        bool,
        typer.Option(
    ...
    try:
        ...
        if not import_method:
            from .importers.timeshift_importer import (
            ...
            try:
                ...
                show_error_panel(
                    "Timeshift Configuration Not Found",
                    "Timeshift configuration file could not be located.",
                )
                raise typer.Exit(1)
            except PermissionError as exc:
                show_error_panel(
                    "Timeshift Configuration Error",
                    f"Permission denied reading Timeshift configuration: {exc}",
                )
                raise typer.Exit(1)
            except json.JSONDecodeError as exc:
                show_error_panel(
                    "Invalid Timeshift Configuration",
                    f"Invalid Timeshift configuration: {exc}",
                )
                raise typer.Exit(1)
            except Exception as exc:
                show_error_panel(
                    "Timeshift Import Error",
                    f"Failed to parse Timeshift configuration: {exc}",
                )
                if verbose:
                    console.print_exception()
                raise typer.Exit(1)
            ...
            result = cast(
                _TimeshiftImportResultLike,
                mapper.import_configuration(
                    parsed_config,
            ...
            config_path_display = summary.get("config_file") or (
                str(config_file) if config_file else "default locations"
            )
            console.print(
                f"[bold]Timeshift Configuration Found:[/bold] {config_path_display}"
            )
            ...
            exclude_patterns = cast(
                list[str], selection_config.get("exclude_patterns", [])
            )
            ...
            console.print(
                f"- Description: {repo_config.get('description', 'Imported from Timeshift')}"
            )
            console.print(
                f"- Backend: {repo_config.get('backend', 'restic (auto-detected)')}"
            )

            console.print("\n[bold]Selection Template Configuration[/bold]")
            console.print(
                f"- Template: {selection_config.get('name', default_selection_name)}"
            )
            ...
            if result.warnings:
                ...
                if str(parsed_config.get("btrfs_mode", "false")).lower() == "true":
                    console.print(
                        "- BTRFS Mode: Yes (Timeshift configuration indicates BTRFS snapshots were enabled.)"
                    )
            ...
            show_success_panel(
                "Timeshift Import", "Timeshift configuration import dry-run completed."
            )
            return
        ...
        if success_flag:
            ...
            show_error_panel(
                "Timeshift Import Failed",
                "Failed to import Timeshift configuration.",
                result.errors,
            )
            raise typer.Exit(1)
    except KeyboardInterrupt:
        ...
        show_error_panel(
            "Timeshift Import Error", f"Failed to import Timeshift configuration: {exc}"
        )
        if verbose:
            console.print_exception()
        raise typer.Exit(1)
...
class UserFacingLogFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if record.levelno >= logging.WARNING:
            ...
            is_user_logger = any(
                logger_name.startswith(prefix.lower())
                for prefix in user_relevant_loggers
            )
            ...
            has_user_keywords = any(
                pattern in message for pattern in user_relevant_patterns
            )
...
def setup_logging(verbose: bool = False, _config_dir: Path | None = None) -> None:
    ...
    logger = logging.getLogger(__name__)
    logger.debug(
        f"Logging configured - Level: {logging.getLevelName(level)}, Log file: {log_file}"
    )
...
def show_success_panel(
    title: str, message: str, details: dict[str, object] | None = None
) -> None:
...
def show_error_panel(
    title: str, message: str, details: list[str] | None = None
) -> None:
...
def _get_service_method(
    manager: object, method_name: str
...
def _call_service_method(
    method: Callable[..., object] | None, **candidates: object
) -> object:
...
def _create_credential_manager(
...
def _create_configuration_module(config_dir: Path | None = None) -> ConfigurationModule:
    ...
    def _is_mock(candidate: object) -> bool:
        return getattr(
            getattr(candidate, "__class__", None), "__module__", ""
...
def _repository_config_to_dict(repository_obj: object, name: str) -> dict[str, object]:
    ...
    if callable(to_dict_method):
        ...
        data = {str(key): value for key, value in source_mapping.items()}
    else:
        data = {"name": name}
        for attr in (
            "uri",
...
def repos_credentials_set(
    name: Annotated[
        str,
        typer.Argument(
            help="Repository name", autocompletion=repository_name_completer
        ),
    ],
    master_password: Annotated[
        ...
        typer.Option(
            "--master-password",
            "-m",
            help="Master password to unlock the credential manager if locked",
        ),
    ] = None,
    verbose: Annotated[
        bool, typer.Option("--verbose", "-v", help="Enable verbose output")
    ...
    try:
        ...
        repo_uri = getattr(repository_obj, "uri", None) or getattr(
            repository_obj, "location", None
        )
        ...
        if backend_type != "s3":
            show_error_panel(
                "Unsupported Backend",
                "Backend credentials management is currently supported for S3 repositories only.",
            )
            raise typer.Exit(1)
        ...
        if repository_factory is not None:
            try:
                setattr(
                    cast(object, repository_factory),
                    "_credential_manager",
                    credential_manager,
                )
            except Exception as attach_exc:
                logging.getLogger(__name__).debug(
                    "Unable to attach credential manager to repository factory: %s",
                    attach_exc,
                )
        ...
        try:
            ...
            secret_key = prompt_service.prompt_password(
                "AWS Secret Access Key", required=True
            )
            region = prompt_service.prompt_text(
                "AWS Region", default="", required=False
            )
            insecure_tls = prompt_service.prompt_confirm(
                "Allow insecure TLS (skip certificate verification)?", default=False
            )
        ...
        show_success_panel(
            "Credentials Stored",
            f"{_backend_display_name(backend_type)} credentials stored for '{name}'.",
        )
        ...
        show_error_panel(
            "Credential Error", f"Failed to store repository credentials: {e}"
        )
        if verbose:
            console.print_exception()
        raise typer.Exit(1)
...
def repos_credentials_remove(
    ...
    yes: Annotated[
        bool, typer.Option("--yes", "-y", help="Confirm removal without prompt")
    ] = False,
    master_password: Annotated[
        str | None,
        typer.Option(
    ...
    try:
        ...
        if backend_type != "s3":
            show_info_panel(
                "Unsupported Backend",
                "No backend credentials stored for this repository type.",
            )
            raise typer.Exit(0)
        ...
        if not confirmed:
            if interactive:
                ...
                confirmed = prompt_service.prompt_confirm(
                    f"Remove {_backend_display_name(backend_type)} credentials for '{name}'?",
                    default=False,
                )
                if not confirmed:
                    show_info_panel(
                        "Operation Cancelled", "Credential removal cancelled."
                    )
                    raise typer.Exit(0)
            else:
                confirmed = True
        ...
        if master_password is not None:
            ...
            try:
                ...
                if interactive:
                    raise
                logging.getLogger(__name__).debug(
                    "Unable to unlock credential manager automatically for remove command."
                )

        removed = False
        if hasattr(credential_manager, "remove_repository_backend_credentials"):
            removed = credential_manager.remove_repository_backend_credentials(
                name, backend_type
            )
        ...
        if removed:
            ...
            try:
                ...
                logging.getLogger(__name__).debug(
                    "Failed to update repository after credential removal: %s", exc
                )
            show_success_panel(
                "Credentials Removed",
                f"Removed {_backend_display_name(backend_type)} credentials for '{name}'.",
            )
        else:
            show_info_panel(
                "No Credentials", f"No stored credentials found for '{name}'."
            )
    except RepositoryNotFoundError as e:
        show_error_panel("Repository Not Found", str(e))
        raise typer.Exit(1)
        ...
        show_error_panel(
            "Credential Error", f"Failed to remove repository credentials: {e}"
        )
        if verbose:
            console.print_exception()
        raise typer.Exit(1)
...
def repos_credentials_show(
    ...
    try:
        ...
        if master_password is not None:
            ...
            try:
                ...
                if interactive:
                    raise
                logging.getLogger(__name__).debug(
                    "Unable to unlock credential manager automatically for show command."
                )

        has_credentials = False
        if hasattr(credential_manager, "has_repository_backend_credentials"):
            has_credentials = credential_manager.has_repository_backend_credentials(
                name, backend_type
            )
        ...
        if not has_credentials:
            show_info_panel(
                "No Credentials",
                f"No {_backend_display_name(backend_type)} credentials stored for '{name}'.",
            )
            return
        ...
        if hasattr(credential_manager, "get_repository_backend_credentials"):
            credentials = (
                credential_manager.get_repository_backend_credentials(
                    name, backend_type
                )
                or {}
            )
        ...
        show_error_panel(
            "Credential Error", f"Failed to display repository credentials: {e}"
        )
        if verbose:
            console.print_exception()
        raise typer.Exit(1)
...
def _ensure_manager_unlocked(
    ...
    if master_password is None:
        if interactive:
            prompt_service = PromptService(console=console)
            try:
                master_password = prompt_service.prompt_password(
                    "Master password", required=True
                )
            except PromptError:
                pass
        if master_password is None:
            show_error_panel(
                ...
                "Provide --master-password to unlock before proceeding.",
            )
            raise typer.Exit(1)
    ...
    if not manager.unlock(master_password):
        show_error_panel(
            "Unlock Failed",
            "Unable to unlock credential manager with the provided master password.",
        )
        raise typer.Exit(1)
...
def config_export_config(
    file: Annotated[
        Path, typer.Argument(help="Output file path for configuration export")
    ],
    include_repositories: Annotated[
        bool,
        typer.Option(
            "--repositories/--no-repositories", help="Include repository configurations"
        ),
    ] = True,
    include_selections: Annotated[
        bool,
        typer.Option(
            "--selections/--no-selections", help="Include data selection configurations"
        ),
    ] = True,
    include_policies: Annotated[
        bool,
        typer.Option("--policies/--no-policies", help="Include policy configurations"),
    ] = True,
    include_schedules: Annotated[
        bool,
        typer.Option(
            "--schedules/--no-schedules", help="Include schedule configurations"
        ),
    ] = True,
    include_credentials: Annotated[
        ...
        typer.Option(
            ...
            help="Include credential references (not actual secrets)",
        ),
    ] = False,
    overwrite: Annotated[
        bool, typer.Option("--overwrite", "-f", help="Overwrite existing file")
    ] = False,
    verbose: Annotated[
        bool, typer.Option("--verbose", "-v", help="Enable verbose output")
    ] = False,
    ...
    try:
        ...
        if include_repositories:
            repo_count = len(
                cast(dict[str, object], export_data.get("repositories", {}))
            )
            summary_parts.append(f"{repo_count} repositories")
        if include_selections:
            selection_count = len(
                cast(dict[str, object], export_data.get("data_selections", {}))
            )
        ...
        if include_schedules:
            schedule_count = len(
                cast(dict[str, object], export_data.get("schedules", {}))
            )
        ...
        show_success_panel(
            ...
            f"Configuration exported to '{file}'\n\nExported: {summary}",
            {
                ...
                "Credentials": "Included (references only)"
        ...
        show_error_panel(
            "Operation Cancelled", "Configuration export cancelled by user"
        )
        raise typer.Exit(130)
    except Exception as e:
...
def migrate_validate(
    source: Annotated[
        Path, typer.Argument(help="Source configuration file to validate")
    ],
    show_changes: Annotated[
        bool, typer.Option("--show-changes", help="Show detailed change summary")
    ] = True,
    check_compatibility: Annotated[
        bool, typer.Option("--check-compatibility", help="Check version compatibility")
    ] = True,
    verbose: Annotated[
        bool, typer.Option("--verbose", "-v", help="Enable verbose output")
    ] = False,
    ...
    try:
        ...
        try:
            ...
            show_error_panel("Invalid JSON", f"Source file contains invalid JSON: {e}")
            raise typer.Exit(2)
        ...
        current_repos = {
            name: repo for name, repo in current_config.repositories.items()
        }
        ...
        for repo_name, repo_data in import_repos.items():
            ...
            if repo_name in current_repos:
                current_uri = getattr(current_repos[repo_name], "uri", None) or getattr(
                    current_repos[repo_name], "location", None
                )
                import_uri = repo_data.get("uri") or repo_data.get("location")
                if current_uri != import_uri:
                    validation_results["changes"]["repositories"]["update"].append(
                        repo_name
                    )
                    repository_warning = (
        ...
        current_targets = {
            name: target for name, target in current_config.backup_targets.items()
        }
        ...
        for target_name, target_data in import_targets.items():
            ...
            if (
                target_repo
                and target_repo not in import_repos
                and target_repo not in current_repos
            ):
        ...
        if import_policies:
            for policy_name, policy_data in import_policies.items():
                ...
                if (
                    policy_repo
                    and policy_repo not in import_repos
                    and policy_repo not in current_repos
                ):
                    validation_results["warnings"].append(
        ...
        if metadata:
            console.print(
                f"[bold]Exported:[/bold] {metadata.get('exported_at', 'unknown')}"
            )
        ...
        if validation_results["valid"]:
            console.print(
                "[bold green]✓ Configuration is valid and can be imported[/bold green]\n"
            )
        else:
            console.print(
                "[bold red]✗ Configuration has errors and cannot be imported[/bold red]\n"
            )
        ...
        if show_changes:
            ...
            total_changes = sum(
                _change_bucket_total(bucket) for _, bucket in change_sections
            )
            ...
            if total_changes == 0:
                ...
                for category, actions in change_sections:
                    ...
                    if category_changes > 0:
                        ...
                        if actions["add"]:
                            console.print(
                                f"    [green]+ Add:[/green] {', '.join(actions['add'])}"
                            )
                        if actions["update"]:
                            console.print(
                                f"    [yellow]~ Update:[/yellow] {', '.join(actions['update'])}"
                            )
                        if actions["remove"]:
                            console.print(
                                f"    [red]- Remove:[/red] {', '.join(actions['remove'])}"
                            )
                console.print()
...
def config_import_config(
    ...
    merge: Annotated[
        bool,
        typer.Option(
            "--merge", help="Merge with existing configuration instead of replacing"
        ),
    ] = True,
    overwrite: Annotated[
        bool, typer.Option("--overwrite", help="Overwrite conflicting items")
    ] = False,
    dry_run: Annotated[
        bool, typer.Option("--dry-run", help="Preview changes without applying them")
    ] = False,
    yes: Annotated[
    ...
    try:
        ...
        if not file.exists():
            show_error_panel(
                "File Not Found", f"Configuration file '{file}' does not exist."
            )
            raise typer.Exit(2)
        ...
        try:
            ...
            show_error_panel(
                "Invalid JSON", f"Configuration file contains invalid JSON: {e}"
            )
            raise typer.Exit(2)
        ...
        if metadata:
            ...
            console.print(
                f"[bold]Version:[/bold] {metadata.get('timelocker_version', 'unknown')}"
            )
        ...
        if not dry_run and not yes and interactive:
            ...
            if not prompt_service.prompt_confirm("Proceed with import?", default=False):
                show_info_panel(
                    "Import Cancelled", "Configuration import cancelled by user"
                )
                raise typer.Exit(0)
        ...
        if dry_run:
            ...
            show_info_panel("Dry Run Complete", dry_run_message)
            raise typer.Exit(0)
        ...
        show_error_panel(
            "Operation Cancelled", "Configuration import cancelled by user"
        )
        raise typer.Exit(130)
    except typer.Exit:
        raise
...
try:
    from .cli_modules.commands.selections import (
        selections_app as _selections_commands_app,
    )
...
try:
    ...
    from .cli_modules.commands.monitoring import runs_app as _runs_commands_app
...
try:
    from .cli_modules.commands.credentials import (
        credentials_app as _credentials_commands_app,
    )
