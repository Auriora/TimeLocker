...
def snapshots_restore(
    ...
    if _evaluate_success(result):
        show_success_panel("Restore Completed", f"Snapshot restored to {target}")
        return

    show_error_panel("Restore Failed", "Snapshot restore did not succeed.")
    raise typer.Exit(1)
...
def snapshots_contents(
    ...
    service = _snapshot_service()
    repo = _normalize_repository(repository)
    entries = service.list_snapshot_contents(
            snapshot_id,
            path=path,
            repository=repo,
    )
    ...
    table = Table(title=f"Snapshot {snapshot_id[:12]} contents")
    ...
    for entry in entries:
        table.add_row(
                getattr(entry, "type", "file"),
                getattr(entry, "path", ""),
                str(getattr(entry, "size", "")),
        )
...
def snapshots_mount(
        snapshot_id: str = typer.Argument(..., help="Snapshot ID"),
        mount_point: Path = typer.Argument(..., help="Mountpoint"),
        repository: Optional[str] = typer.Option(
                None,
                "--repository",
                "-r",
    ...
    """Mount a snapshot as a read-only filesystem at the given mount point."""
    _validate_snapshot_id(snapshot_id, allow_latest=True)
    service = _snapshot_service()
    repo = _normalize_repository(repository)
    result = service.mount_snapshot(
            snapshot_id=snapshot_id,
            repository=repo,
            mount_point=str(mount_point),
    )
    if _evaluate_success(result):
        show_success_panel("Snapshot Mounted", f"Mounted at {mount_point}")
        return
    show_error_panel("Mount Failed", "Snapshot mount did not succeed.")
    raise typer.Exit(1)
...
def snapshots_umount(
    ...
    service = _snapshot_service()
    repo = _normalize_repository(repository)
    result = getattr(service, "unmount_snapshot", None)
    if result is None:
        show_error_panel("Mount Error", "Snapshot unmount is not available.")
        raise typer.Exit(1)
    outcome = result(snapshot_id=snapshot_id, repository=repo)
    if _evaluate_success(outcome):
        show_success_panel("Snapshot Unmounted", f"Unmounted snapshot {snapshot_id[:12]}")
        return
    show_error_panel("Unmount Failed", "Snapshot unmount did not succeed.")
    raise typer.Exit(1)
