...
def _get_console() -> Console:
    """Get a console instance that works with testing"""
    import typer
    try:
        # Use typer's stdout for test compatibility
...
def export_selection(
    ...
    try:
        ...
        if output is None:
            output = Path(f"{name}.{format}")
        
        # Export template
        exported_data = asyncio.run(template_manager.export_templates([template.id], format=format))
        ...
        _show_success(
            "Selection Exported",
            f"Selection template '{name}' exported to {output}"
        )
...
def import_selection(
    ...
    try:
        ...
        # Import templates
        template_manager = SelectionTemplateManager()
        result = asyncio.run(template_manager.import_templates(data, format=format, merge_strategy=merge))
