...
def load_minio_settings(
    ...
    if endpoint_value:
        ...
        settings.setdefault("MINIO_URI_PREFIX", f"s3:{url}")
    ...
    for config_path in config_candidates:
        ...
        for repo in config_data.get("repositories", []):
            ...
            if host and "MINIO_ENDPOINT_HOST" not in settings:
                ...
                settings["MINIO_ENDPOINT_URL"] = settings.get(
                    "MINIO_ENDPOINT_URL", f"http://{normalized_host}"
                )
                settings.setdefault("AWS_S3_ENDPOINT", settings["MINIO_ENDPOINT_URL"])
                settings.setdefault(
                    "MINIO_URI_PREFIX", f"s3:{settings['MINIO_ENDPOINT_URL']}"
                )
            ...
            if isinstance(creds, dict):
                settings.setdefault(
                    "MINIO_ACCESS_KEY", creds.get("aws_access_key_id", "")
                )
                settings.setdefault(
                    "MINIO_SECRET_KEY", creds.get("aws_secret_access_key", "")
                )
        ...
        missing_core = [
            key
            for key in ("MINIO_ENDPOINT_HOST", "MINIO_BUCKET")
    ...
    settings.setdefault(
        "MINIO_VERIFY_SSL",
        os.getenv("MINIO_VERIFY_SSL", settings.get("MINIO_VERIFY_SSL", "true")),
    )
    ...
    if "MINIO_URI_PREFIX" not in settings and "MINIO_ENDPOINT_HOST" in settings:
        settings["MINIO_URI_PREFIX"] = f"s3:{settings['MINIO_ENDPOINT_URL']}"
...
def ensure_minio_reachable(
    ...
    try:
        with urllib.request.urlopen(endpoint, timeout=5) as response:
            if response.status >= 500:
                raise RuntimeError(
                    f"MinIO endpoint responded with status {response.status}"
                )
    ...
    if last_error:
        raise RuntimeError(
            f"Unable to reach MinIO endpoint '{endpoint}': {last_error}"
        ) from last_error
