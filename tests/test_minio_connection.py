...
def sanitize_endpoint(endpoint: str) -> str:
    """
    Sanitize endpoint URL for display to avoid exposing sensitive information.

    Removes query parameters and fragments that might contain auth tokens or API keys.

    Args:
        endpoint: The endpoint URL to sanitize

    Returns:
        Sanitized endpoint URL with only scheme, netloc, and path
    """
...
def test_s3_repository(access_key, secret_key, endpoint):
    ...
    try:
        ...
        print(f"✓ Set AWS_S3_ENDPOINT={sanitize_endpoint(endpoint)}")
        ...
        print(f"   AWS_S3_ENDPOINT: {sanitized}")

        print(f"\n✅ SUCCESS! S3ResticRepository configured correctly")
        print(f"\nTo use with TimeLocker, ensure:")
        print(f"  1. Set environment variable: export AWS_S3_ENDPOINT={sanitize_endpoint(endpoint)}")
...
def main():
    ...
    print("\nNext steps:")
    print(f"  1. Set environment: export AWS_S3_ENDPOINT={sanitize_endpoint(endpoint)}")
